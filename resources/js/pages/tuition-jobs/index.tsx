...
function classLabel(value: string | null, t: T) {
    if (!value) return '';
    if (value === 'nursery') return str(t?.card?.class_nursery, 'Nursery');
    if (value === 'kg') return str(t?.card?.class_kg, 'KG');
...
function TuitionCard({ post, t }: { post: Post; t: T }) {
    ...
    const classLevels = [...new Set(post.students.map((s) => classLabel(s.class_level, t)).filter(Boolean))];
    ...
    return (
        <Link href={`/tuition-posts/${post.id}`} className="group flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-blue-300 hover:shadow-md">
            <div className="flex items-start justify-between gap-4">
                ...
                <span className="shrink-0 rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">{salaryLabel(post, t)}</span>
            </div>
            ...
            {classLevels.length > 0 && (
                <div className="flex flex-wrap gap-2">
                    {classLevels.slice(0, 3).map((label) => (
                        <span key={label} className="rounded-md bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 border border-amber-100/60">
                            {label}
                        </span>
                    ))}
                </div>
            ...
            <div className="mt-auto pt-4 border-t border-slate-100 flex flex-wrap gap-y-3 gap-x-5 text-sm text-slate-500">
                ...
                {post.tutor_gender_preference !== 'any' && (
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="capitalize">{replaceVars(str(t?.tuition_jobs?.tutor_suffix, ':gender Tutor'), { gender: post.tutor_gender_preference })}</span>
                    </div>
                )}
            </div>
        </Link>
...
export default function TuitionJobsPage({
    ...
    filters: { location: string; class_level: string; academic_group: string; gender: string; level: string; min_salary: number | null; max_days: number | null };
    canRegister?: boolean;
}) {
            ...
            <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
                ...
                <div className="mt-8 flex flex-col gap-8 lg:flex-row">
                        ...
                        <form onSubmit={onSubmit} className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            ...
                            <div className="space-y-4">
                                <AutocompleteInput
                                    ...
                                    className="w-full min-w-0 rounded-xl border border-slate-300 pl-10 px-4 py-2.5 text-sm"
                                />
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        ...
                                        className="w-full min-w-0 appearance-none rounded-xl border border-slate-300 px-4 py-2.5 pl-10 pr-10 text-sm"
                                    >
                                        <option value="">{str(t?.tuition_jobs?.placeholder_class, 'Select Class')}</option>
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        ...
                                        className="w-full min-w-0 appearance-none rounded-xl border border-slate-300 px-4 py-2.5 pl-10 pr-10 text-sm disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                                    >
                                        <option value="">{shouldShowGroup ? str(t?.tuition_jobs?.placeholder_group, 'Select Group') : 'Select Group (class 9-12 only)'}</option>
                                ...
                                <select value={gender} onChange={(e) => setGender(e.target.value)} className="w-full min-w-0 rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                                ...
                                <select value={level} onChange={(e) => setLevel(e.target.value)} className="w-full min-w-0 rounded-xl border border-slate-300 px-4 py-2.5 text-sm">
                                ...
                                <input
                                    ...
                                    placeholder={str(t?.tuition_jobs?.placeholder_min_salary, 'Minimum Budget (BDT)')}
                                    type="number"
                                    min={0}
                                ...
                                <input
                                    ...
                                    placeholder={str(t?.tuition_jobs?.placeholder_max_days, 'Max Days/Week')}
                                    type="number"
                                    min={1}
                                    max={7}
                                    className="w-full min-w-0 rounded-xl border border-slate-300 px-4 py-2.5 text-sm"
                            ...
                            <div className="mt-5 flex gap-2">
                                ...
                                <Link href="/tuition-jobs" className="rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-200">
                    ...
                    <main className="min-w-0 flex-1">
                        <div className="text-sm font-medium text-slate-600">{replaceVars(str(t?.tuition_jobs?.posts_found, ':count posts found'), { count: posts.total })}</div>
                        ...
                        {posts.data.length === 0 ? (
                            ...
                            <div className="mt-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                                {posts.data.map((post) => (
                                    <TuitionCard key={post.id} post={post} t={t} />
                                ))}
                            </div>
                        )}
                                    ...
                                    {posts.links.map((link, i) =>
                                        link.url ? (
                                            <button
                                                ...
                                                onClick={() => {
                                                    setIsFiltering(true);
                                                    router.visit(link.url!, { preserveState: true, onFinish: () => setIsFiltering(false) });
                                                }}
                                                className={`px-4 py-2 text-sm cursor-pointer ${link.active ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
